...
test.describe('글 작성 브라우저 흐름', () => {
	...
	test('뒤로가기를 취소하면 작성 내용을 유지하고 확인하면 이전 페이지로 이동한다', async ({ page }) => {
	...
	test('새로고침을 취소하면 작성 내용을 유지한다', async ({ page }) => {
		await page.goto('/write');
		const title = page.getByRole('textbox', { name: '게시글 제목' });
	...
	test('파일 선택으로 업로드한 이미지를 편집기에 표시한다', async ({ page }) => {
		await page.goto('/write');
...
test.describe('모바일 글쓰기 정책', () => {
	...
	test('모바일 기기에서는 편집기 대신 PC 이용 안내를 제공한다', async ({ page }) => {
		await enableWriteAccess(page);
		await page.goto('/write');
