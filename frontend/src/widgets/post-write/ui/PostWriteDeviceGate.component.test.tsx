...
vi.mock('./PostWriteLoader', () => ({
	default: () => <div>글쓰기 로더</div>,
}));
...
describe('PostWriteDeviceGate', () => {
	...
	it('모바일 판정 중에는 중립적인 상태를 안내한다', () => {
		...
		expect(screen.queryByText('글쓰기 로더')).not.toBeInTheDocument();
	});
	...
	it('데스크톱에서만 글쓰기 loader를 불러온다', async () => {
		useMobileDeviceMock.mockReturnValue({ isMobileDevice: false, isResolved: true });

		render(<PostWriteDeviceGate />);

		expect(await screen.findByText('글쓰기 로더')).toBeInTheDocument();
	});
});
