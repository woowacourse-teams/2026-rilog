		...
		<div className="relative flex min-h-96 flex-col items-center justify-center overflow-hidden bg-brand-primary px-5 py-12 text-center text-on-brand-primary sm:min-h-112 sm:px-6 sm:py-14 md:min-h-128 md:py-16">
			<div className="relative flex flex-col items-center">
				...
				<div className="relative mt-2.5 inline-block max-w-full">
					<h1 className="max-w-full min-w-0 text-title-2 font-semibold wrap-break-word sm:text-title-3">
						{profile.name}
					</h1>
					{action ? (
						<div className="absolute top-1/2 left-full ml-2 -translate-y-1/2 text-on-brand-primary">{action}</div>
					) : null}
				</div>
				{hasDescription || hasServiceUrl ? (
					<p className="mt-2 max-w-sm text-label-2 text-on-brand-primary sm:mt-2.5 sm:max-w-lg sm:text-body-1">
