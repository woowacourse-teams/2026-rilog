...
describe('parseBlogHomeFilter', () => {
	...
	it.each([
		[{ series: ['3', '4'] }, 'RILOG'],
		[{ colog: ['rilog_team', 'another_team'] }, 'RILOG'],
		[{ series: '3', colog: 'rilog_team' }, 'RILOG'],
		[{ series: '0' }, 'RILOG'],
		[{ chapter: '3.5' }, 'COLOG'],
		[{ colog: '잘못된 slug' }, 'RILOG'],
		[{ chapter: '3' }, 'RILOG'],
		[{ series: '3' }, 'COLOG'],
		[{ colog: 'rilog_team' }, 'COLOG'],
		[{ colog: '@rilog_team' }, 'RILOG'],
		[{ colog: 'RILOG_TEAM' }, 'RILOG'],
		[{ chapterId: '3' }, 'RILOG'],
		[{ targetCologSlug: 'rilog_team' }, 'RILOG'],
...
describe('buildBlogHomeFilterHref', () => {
	it('인덱스 필터만 제거하고 다른 query와 중복 값은 보존한다', () => {
		const searchParams = new URLSearchParams('series=3&chapter=3&colog=old_team&tab=recent&notice=one&notice=two');

		expect(
			buildBlogHomeFilterHref(
				'/@rilog',
				searchParams,
				{ type: 'targetCologSlug', targetCologSlug: 'rilog_team' },
				'RILOG',
			),
		).toBe('/@rilog?tab=recent&notice=one&notice=two&colog=rilog_team');
	});
...
describe('isBlogHomeFilterAvailable', () => {
	it('profile type과 인덱스 소속을 함께 사용해 필터를 검증한다', () => {
		...
		expect(isBlogHomeFilterAvailable({ type: 'targetCologSlug', targetCologSlug: 'rilog_team' }, INDEX, 'RILOG')).toBe(
			true,
		);
		expect(isBlogHomeFilterAvailable({ type: 'targetCologSlug', targetCologSlug: 'rilog_team' }, INDEX, 'COLOG')).toBe(
			false,
		);
	});
});
