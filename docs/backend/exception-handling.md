...
| 타입 | 책임 |
| --- | --- |
| `ErrorInformation` | 모든 오류 정보가 제공해야 하는 HTTP 상태와 메시지 계약 |
...
- [ ] 오류 응답 구조 변경이 프론트엔드에 미치는 영향을 공유했는가?
...
- HTTP 상태 또는 응답 필드를 바꾸는 작업은 프론트엔드에도 영향을 주는 API 계약 변경으로 취급한다.
